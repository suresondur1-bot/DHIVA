    ...
    for var in (variables or []):
        ...
        try:
            ...
            if vtype == "random_email":
                ...
                items = ([str(x).strip() for x in raw if str(x).strip()] if isinstance(raw, list)
                         else [x.strip() for x in str(raw).split(",") if x.strip()])
    ...
    try:
        ...
        # ── SEARCH_SELECT ─────────────────────────────────────────────────────
            ...
            for _attempt in range(17):
                try:
                    if await page.locator("ng-dropdown-panel .ng-option").count() > 0:
                    ...
                    if n > 0:
                        ...
                        if await filtered.count() > 0:
                            ...
                            for j in range(min(n, 20)):
                                o = opts.nth(j)
                                try:
                                    txt = (await o.inner_text(timeout=500) or "").strip()
...
async def run_steps_with_flow(page, steps, run_id, resolved_vars, config,
    ...
    while i < len(steps):
        ...
        if action in ("if", "if_start"):
            ...
            # ── Map frontend if_var/if_value/if_condition → left/right/operator ──
            ...
            if if_condition in ("var_equals", "var_not_equals", "var_contains"):
                ...
                left     = apply_variables(step.get("if_value", ""), resolved_vars)
                right    = ""
                operator = if_condition
            elif if_condition == "page_title_contains":
                left     = apply_variables(step.get("if_value", ""), resolved_vars)
                right    = ""
                operator = if_condition
            elif if_condition in ("element_visible", "element_not_visible"):
                left     = apply_variables(step.get("if_selector", "") or step.get("selector", ""), resolved_vars)
                right    = ""
                operator = if_condition
            else:
            ...
            while i < len(steps):
                ...
                if a in ("if", "if_start"):
                    depth += 1
                if a in ("end_if", "if_end") and depth == 0:
                    i += 1
                    break
                if a == "else" and depth == 0:
                    skip = not skip
                    i += 1
                    continue
                if a in ("end_if", "if_end") and depth > 0:
                    depth -= 1
                if not skip:
                    sub = await run_steps_with_flow(page, [s], run_id, resolved_vars, config,
        ...
        # ── MATH ──────────────────────────────────────────────────────────────
        if action in ("math_add","math_subtract","math_multiply","math_divide","math_round","math_abs"):
            var_name = apply_variables(step.get("store_as","") or step.get("value",""), resolved_vars).strip()
            try:
        ...
        if action in ("date_today","date_now","date_add","date_subtract","date_format","date_diff"):
            ...
            try:
                ...
                if   action == "date_today":    result_val = _dt.now().strftime(fmt)
                    ...
                    in_fmt = apply_variables(step.get("value3","") or "%d/%m/%Y", resolved_vars)
                    result_val = str(abs((_dt.strptime(val2, in_fmt) - _dt.strptime(val1, in_fmt)).days))
                else: result_val = ""
                if var_name: resolved_vars[var_name] = result_val
                log(run_id,"pass",f"[OK] {action}: '{result_val}' → {{{{{var_name}}}}}")
                results.append({"status":"passed","step":i})
        ...
        if action in ("str_upper","str_lower","str_trim","str_replace","str_substring",
            ...
            try:
                ...
                if   action == "str_upper":     result_val = src.upper()
                    ...
                    parts_s = v2.split(",")
                    start = int(parts_s[0]) if parts_s else 0
                    end   = int(parts_s[1]) if len(parts_s)>1 else len(src)
