...
# ═══════════════════════════════════════════════════════════════════════════
# COPY THIS CODE INTO: C:\Users\337799\Automation\runner\runner.py
...
def validate_api_assertion(response, assertion, variables):
    """Validate API assertions - 13 types supported"""
    ...
    if assertion_type == 'status_code':
        ...
        assert text in response.text, f"Response missing '{text}'"
    
    elif assertion_type == 'json_key_exists':
        data = response.json()
        value = get_json_path(data, assertion['key'])
        assert value is not None, f"Key '{assertion['key']}' not found"
    
    elif assertion_type == 'json_value':
        data = response.json()
        expected = assertion['value']
        actual = get_json_path(data, assertion['key'])
        assert str(actual) == str(expected), f"Expected '{expected}', got '{actual}'"
    
    elif assertion_type == 'response_time':
        max_ms = int(assertion['value'])
        actual_ms = int(response.elapsed.total_seconds() * 1000)
        assert actual_ms < max_ms, f"{actual_ms}ms > {max_ms}ms"
        ...
        actual = response.headers.get(assertion['key'])
        assert actual == assertion['value'], "Header mismatch"
    
    elif assertion_type == 'json_array_length':
        data = response.json()
        arr = get_json_path(data, assertion['path'])
        assert isinstance(arr, list), "Not an array"
        assert len(arr) == int(assertion['length']), "Length mismatch"
        ...
        print(f"[EXTRACT] {assertion['variable']} = {variables[assertion['variable']]}")
...
# ── HEAL CACHE — loaded once at startup, dictionary lookup per step (zero DB calls during run)
_heal_cache: dict = {}  # original_selector -> healed_selector

def _load_heal_cache():
    """Load all healed selectors from DB into memory once at startup."""
    global _heal_cache
    if not RUNNER_TOKEN:  # skip if token not set yet
        return
    try:
        resp = requests.get(
        ...
        if resp.status_code == 200:
            _heal_cache = resp.json()  # {original: healed} map
            print(f"[heal cache] loaded {len(_heal_cache)} entries from DB")
    except Exception as e:
        print(f"[heal cache] could not load from DB: {e} (will start empty)")

# DO NOT call _load_heal_cache() here -- token not set yet, will be called in main() after token is loaded
...
def resolve_variables(variables):
    ...
    for var in (variables or []):
        ...
        try:
            ...
            # PARALLEL FIX: Check for 'value' field first (parallel runs), then fall back to config
...
def apply_variables(value, resolved):
    ...
    for name, val in resolved.items():
        # Always convert to string -- val can be dict/list/int/None if a variable
        # config was stored as a JSON object (e.g. {"value": "something"})
        if isinstance(val, (dict, list)):
            import json as _j
            str_val = _j.dumps(val)
        elif val is None:
            str_val = ""
        else:
            str_val = str(val)
...
def log(run_id, level, message, step_index=None):
    ...
    # Always print to console (flush immediately for live log streaming)
    ...
    # Send to backend API -- non-blocking background thread
    def _post():
        try:
            requests.post(f"{API_BASE}/api/runs/{run_id}/log", json=payload, timeout=3)
        except Exception:
            pass
...
_last_frame_hash = None   # MD5 of last sent screenshot bytes
_last_live_ts    = 0      # timestamp of last sent screenshot
_LIVE_MIN_GAP    = 1.0    # minimum seconds between sends (1s)

def _hash_bytes(b):
    return hashlib.md5(b).hexdigest()

def _post_live_async(run_id, b64, label):
    """Send live screenshot to backend in a background thread."""
    def _post():
        try:
            requests.post(f"{API_BASE}/api/runs/{run_id}/live-screen",
                          json={"data": b64, "label": label,
                                "timestamp": datetime.utcnow().isoformat()},
                          timeout=3)
        except Exception:
            pass
...
def take_live_screenshot(page, run_id, step_label, force=False):
    """Capture and send a live screenshot -- skips duplicates and throttles.
    
    - Hash-dedup: identical frames are never sent.
    - Throttle: min 300ms between sends (unless force=True).
    - Always runs on the main Playwright thread.
    """
    global _last_frame_hash, _last_live_ts
    now = time.time()
    if not force and (now - _last_live_ts) < _LIVE_MIN_GAP:
        return  # Too soon
    try:
        raw = page.screenshot(full_page=False, type="jpeg", quality=60)
    except Exception:
        return  # Page may be navigating
    frame_hash = _hash_bytes(raw)
    if frame_hash == _last_frame_hash and not force:
        return  # Identical frame -- skip
    _last_frame_hash = frame_hash
    _last_live_ts    = now
    b64 = base64.b64encode(raw).decode()
    _post_live_async(run_id, b64, step_label)

def take_step_screenshots(page, run_id, step_label):
    """Capture before+after screenshots around an action step.
    Call BEFORE the action, then again AFTER."""
    take_live_screenshot(page, run_id, step_label, force=False)

def take_step_screenshot_after(page, run_id, step_label, settle_ms=300):
    """Called right after an action -- waits briefly for page to settle,
    then sends a screenshot only if it changed."""
    if settle_ms > 0:
        try:
            page.wait_for_load_state("domcontentloaded", timeout=settle_ms)
        except Exception:
            pass  # Timeout is fine -- page may already be loaded
        time.sleep(min(settle_ms / 1000, 0.3))
    take_live_screenshot(page, run_id, step_label, force=False)

# ── Continuous capture (no-op -- Playwright is single-threaded) ───────────────
def start_screen_capture(page, run_id, interval=1.5):
    """No-op: background threads cause Playwright greenlet conflicts.
    Screenshots are captured per-step in the main thread instead."""
    pass
...
def simplify_selector(sel):
    """Simplify a long brittle CSS selector into a shorter stable one."""
    import re as _re
...
def get_locator_with_fallback(page, selector, timeout=5000):
    """Try primary selector then multiple fallback strategies before AI heal."""
    import re as _re
    ...
    # Fallback 1: simplify long CSS path
    ...
    # Fallback 2: extract text from :has-text() and use get_by_text
    ...
    # Fallback 3: last meaningful CSS segment
    ...
    # Fallback 4: for li > button patterns -- wait for page to stabilise then retry
    # This handles Angular layout shifts where tabs load and push buttons down
    if "button" in selector or "btn" in selector:
        try:
            page.wait_for_timeout(800)  # wait for layout to settle
            loc = get_locator(page, selector)
            loc.wait_for(state="attached", timeout=timeout)
            return loc
        except Exception:
            pass
        # Also try simplified version after wait
        if simple and simple != selector:
            try:
                loc = get_locator(page, simple)
                loc.wait_for(state="attached", timeout=timeout)
    ...
    # Nothing worked -- return original so caller gets the proper error
...
def ai_heal_step(page, step, run_id, idx, original_error, resolved_vars=None):
    ...
    for i, sug in enumerate(suggestions):
        ...
        try:
            ...
            if action == "click":
                # JS click to bypass ng-scrollbar pointer-events interception
                try:
...
def _run_step_impl(page, step, run_id, idx, resolved_vars=None):
    ...
    step_desc = {
        ...
        "press_sequentially": f"Typing '{value}' into debounce field: [ {selector} ]",
        "compare_pdf_page":   f"Comparing PDF '{value}' with page content: [ {selector} ]",
        "press_sequentially": f"Typing letter by letter '{value}' into: [ {selector} ]",
        "wait":               f"Waiting for {value} milliseconds...",
        "table_action":       f"Table Action: find '{step.get('search_text','')}' in col {step.get('search_col',1)}, {step.get('sub_action','click')} col {step.get('target_col',1)}",
        ...
        "scroll":             f"Scrolling page — Y={value or 0} X={step.get('value2', 0) or 0} px",
    ...
    # Log step description/note if provided
    step_note = step.get("description", "") or ""
    if step_note.strip():
        log(run_id, "info", f"   📝 {step_note.strip()}", idx)
    ...
    try:
        if action == "navigate":
            ...
            try:
                ...
                pass  # networkidle timeout ok for apps with continuous polling

        elif action == "click":
            # Normalize absolute href to relative -- actual DOM has relative href
            if selector and selector.startswith('a[href="http'):
                import re as _re_href
                m = _re_href.match(r'a\[href="https?://[^/]+(/[^"]*)"\]', selector)
                if m:
                    selector = f'a[href="{m.group(1)}"]'
                    log(run_id, "info", f"Normalized href to relative: {selector}", idx)

            # For has-text selectors -- use JS exact match (most reliable)
            # Same as: Array.from(querySelectorAll(tag)).find(b => b.textContent.trim() === text).click()
            if selector and ":has-text(" in selector:
                import re as _re_ht
                ht_m = _re_ht.search(r':has-text\(["\'](.+?)["\']\)', selector)
                if ht_m:
                    tag_ht = selector[:ht_m.start()].strip() or "button"
                    txt_ht = ht_m.group(1).strip()
                    # Wait for button to appear
                    try:
                        page.wait_for_selector(tag_ht, timeout=10000)
                    except Exception:
                        pass
                    page.wait_for_timeout(500)
                    # JS exact match -- throws if not found (no silent fail)
                    js = (
                        f"(() => {{"
                        f"  const el = Array.from(document.querySelectorAll(\'{tag_ht}\')).find(b => b.textContent.trim() === \'{txt_ht}\');"
                        f"  if (!el) throw new Error(\'Button not found: {txt_ht}\');"
                        f"  el.click();"
                        f"  return true;"
                        f"}})()"
                    )
                    page.evaluate(js)
                    page.wait_for_timeout(1000)  # wait for navigation to start
                    return {"status": "passed", "step": idx}
            # For href links -- use JS click to avoid ng-scrollbar interception issues
            if selector and 'href=' in selector:
                import re as _re_href
                # Normalize absolute to relative href
                selector = _re_href.sub(r'href="https?://[^/]+/', 'href="/', selector)
                m = _re_href.search(r'href="([^"]+)"', selector)
                if m:
                    href_val = m.group(1)
                    js_href = f"""(() => {{
                        const el = document.querySelector('a[href="{href_val}"]') ||
                                   Array.from(document.querySelectorAll('a')).find(a => a.getAttribute('href') === '{href_val}');
                        if (!el) throw new Error('Link not found: {href_val}');
                        el.click(); return true;
                    }})()"""
                    try:
                        page.wait_for_selector(f'a[href="{href_val}"]', timeout=10000)
                    except Exception:
                        pass
                    page.evaluate(js_href)
                    try:
                    ...
                    page.wait_for_timeout(1500)
                    return {"status": "passed", "step": idx}
            _click_loc = get_locator_with_fallback(page, selector, timeout=min(timeout,8000))
            # Scroll into view and wait for layout to settle before clicking
            # This handles Angular apps where tabs/headers load and shift button positions
            try:
                _click_loc.scroll_into_view_if_needed(timeout=3000)
                page.wait_for_timeout(300)  # brief pause for layout to stabilise
            except Exception:
                pass
            try:
                _click_loc.click(timeout=timeout)
            except Exception as _click_err:
                # Layout shift fallback -- wait longer and retry
                page.wait_for_timeout(1000)
                _click_loc.click(timeout=timeout, force=True)
            # Note: href and has-text clicks return early above via JS click path

        elif action == "type":
            get_locator_with_fallback(page, selector, timeout=min(timeout,8000)).type(value, timeout=timeout)
            ...
            get_locator(page, selector).select_option(value, timeout=timeout)
                ...
                is_ng_select = False
                try:
                    # Fast path: selector string contains ng-select → it IS ng-select
                ...
                if is_ng_select:
                    ...
                    # Find the inner search input — scope to loc first to avoid picking wrong input
                    typed = False
                    # Priority 1: scoped to the specific ng-select locator
                    for ng_inp_sel in [".ng-input > input", "input[type=text]"]:
                        try:
                            ...
                            if ng_inp.is_visible(timeout=500):
                                ...
                                typed = True
                                log(run_id, "info", f"[search_select] typed into ng-select via scoped {ng_inp_sel}")
                                break
                        except Exception:
                            continue
                        ...
                        for ng_inp_sel in [
                            ...
                            try:
                                ...
                                if ng_inp.is_visible(timeout=500):
                                    ...
                                    ng_inp.press_sequentially(search_text)
                                    typed = True
                                    log(run_id, "info", f"[search_select] typed into ng-select via global {ng_inp_sel}")
                                    break
                            except Exception:
                                continue
                    if not typed:
                ...
                # ng-select needs mousedown event -- regular click / force=True do not trigger it
                # ngb-typeahead and other dropdowns work fine with force=True
                _panel_visible = page.locator("ng-dropdown-panel").count() > 0
                if _panel_visible:
                    # ng-select dropdown -- use dispatch_event sequence
                    try:
                        matched.dispatch_event("mousedown")
                        page.wait_for_timeout(50)
                        matched.dispatch_event("mouseup")
                        page.wait_for_timeout(50)
                        matched.dispatch_event("click")
                        log(run_id, "info", "[search_select] ng-select: dispatched mousedown+mouseup+click")
                    except Exception as _de:
                        log(run_id, "info", f"[search_select] dispatch_event failed: {_de}, fallback force click")
                        matched.click(force=True, timeout=5000)
                else:
                    # Other dropdown types (ngb-typeahead etc) -- force click works fine
            ...
            loc = get_locator(page, selector)
            loc.scroll_into_view_if_needed(timeout=timeout)  # scrolls in ALL directions (up/down/left/right)
            ...
            # For debounced inputs (e.g. inputDebounce):
            # fill() types the value and fires input events
            # then we wait for the debounce timeout to trigger the search
            loc = get_locator(page, selector)
            loc.wait_for(state="visible", timeout=timeout)
            loc.click(timeout=timeout)
            page.wait_for_timeout(100)
            ...
            # Dispatch input + change events explicitly for Angular
            try:
                page.evaluate(f"""
                    () => {{
                        const el = document.querySelector({repr(selector)});
            ...
            # Table Action: find row by text in search_col, then act on target_col
            # step fields: selector=table_selector, search_text=text_to_find,
            #              search_col=1, target_col=7, sub_action=click/type/assert_text/get_text, value=text_to_type
            import re as _re_ta
            table_sel   = selector
            search_text = apply_variables(step.get("search_text", ""), resolved_vars)
            ...
            sub_value   = apply_variables(step.get("sub_value", value), resolved_vars)
            ...
            for ri in range(row_count):
                try:
                    cell = rows.nth(ri).locator(f"td:nth-child({search_col})")
                    cell_text = (cell.inner_text(timeout=2000) or "").strip()
            ...
            if found_row == -1:
                raise Exception(
                    f"table_action: Row containing '{search_text}' not found in column {search_col} of [{table_sel}]"
                )

            # Build locator for target cell
            target_cell = rows.nth(found_row).locator(f"td:nth-child({target_col})")
            log(run_id, "info", f"[table_action] Found '{search_text}' at row {found_row+1}, acting on col {target_col}", idx)

            if sub_action == "click":
                target_cell.click(timeout=timeout)
            elif sub_action == "type":
                inp = target_cell.locator("input, textarea").first
                inp.fill(sub_value, timeout=timeout)
            elif sub_action == "select":
                sel_el = target_cell.locator("select").first
                sel_el.select_option(sub_value, timeout=timeout)
            elif sub_action == "search_select":
                inp = target_cell.locator("input, ng-select input").first
                inp.fill(sub_value, timeout=timeout)
                page.wait_for_timeout(1500)
                opt = page.locator(f"[role='option']:has-text('{sub_value}'), .ng-option:has-text('{sub_value}')").first
                opt.click(timeout=timeout)
            elif sub_action == "get_text":
                txt = (target_cell.inner_text(timeout=timeout) or "").strip()
                store_var = step.get("store_as", "")
                if store_var and resolved_vars is not None:
                    resolved_vars[store_var] = txt
                log(run_id, "info", f"[table_action] cell text = '{txt}'", idx)
            elif sub_action == "assert_text":
                txt = (target_cell.inner_text(timeout=timeout) or "").strip()
                assert sub_value.lower() in txt.lower(), \
                    f"table_action assert_text: expected '{sub_value}' in '{txt}'"
            elif sub_action == "assert_visible":
                assert target_cell.is_visible(timeout=timeout), \
                    f"table_action assert_visible: cell at col {target_col} not visible"


        elif action == "table_multi_action":
            # table_multi_action: find rows matching multiple conditions, then run multiple actions
            # Fields:
            #   selector      -- CSS selector for the table
            #   conditions    -- list of {col, value, mode} -- col is 1-based
            #   actions       -- list of {col, sub_action, value, store_as}
            #   match_logic   -- "all" (AND) or "any" (OR), default "all"
            #   which_match   -- "first", "second", "third", "all", default "first"

            table_sel    = selector
            conditions   = step.get("conditions", [])
            actions_list = step.get("actions", [])
            match_logic  = step.get("match_logic", "all").lower()   # "all" or "any"
            which_match  = step.get("which_match", "first").lower() # "first","second","third","all"

            which_map = {"first": 0, "second": 1, "third": 2, "fourth": 3, "fifth": 4}

            # Resolve variables in condition values
            resolved_conditions = []
            for c in conditions:
                resolved_conditions.append({
                    "col":   int(c.get("col", 1)),
                    "value": apply_variables(str(c.get("value", "")), resolved_vars),
                    "mode":  c.get("mode", "contains").lower(),
                })
            ...
            for ri in range(row_count):
                try:
                    ...
                    for cond in resolved_conditions:
                        ...
                        mode = cond["mode"]
                        if mode == "equals":
                            match = cv.lower() == cell_text.lower()
                        elif mode == "startswith":
                            match = cell_text.lower().startswith(cv.lower())
                        else:  # contains
                            match = cv.lower() in cell_text.lower()
                        results.append(match)

                    if match_logic == "any":
                        row_matches = any(results)
                    else:
                        row_matches = all(results)

                    if row_matches:
                        matched_rows.append(ri)
                except Exception:
                    continue

            if not resolved_conditions:
                raise Exception(f"table_multi_action: No conditions defined -- add at least one condition to find a row")

            if not resolved_actions:
                raise Exception(f"table_multi_action: No actions defined -- add at least one action to perform")

            if not matched_rows:
                cond_desc = ", ".join([f"col{c['col']}={c['value']}" for c in resolved_conditions])
                raise Exception(f"table_multi_action: No rows matched conditions [{cond_desc}] in [{table_sel}]")

            log(run_id, "info", f"[table_multi_action] {len(matched_rows)} row(s) matched", idx)

            # Determine which rows to act on
            if which_match == "all":
                target_rows = matched_rows
            else:
                n = which_map.get(which_match, 0)
                if n >= len(matched_rows):
                    raise Exception(
                        f"table_multi_action: which_match='{which_match}' but only {len(matched_rows)} row(s) matched"
                    )
                target_rows = [matched_rows[n]]

            # Execute actions on each target row
            for row_idx in target_rows:
                log(run_id, "info", f"[table_multi_action] Acting on row {row_idx + 1}", idx)
                for act in resolved_actions:
                    col        = act["col"]
                    sub_action = act["sub_action"]
                    sub_value  = act["value"]
                    store_as   = act["store_as"]

                    target_cell = rows.nth(row_idx).locator(f"td:nth-child({col})")

                    if sub_action == "click":
                        target_cell.click(timeout=timeout)
                        log(run_id, "info", f"[table_multi_action] row {row_idx+1} col {col} clicked", idx)

                    elif sub_action == "type":
                        inp = target_cell.locator("input, textarea").first
                        inp.fill(sub_value, timeout=timeout)
                        log(run_id, "info", f"[table_multi_action] row {row_idx+1} col {col} typed '{sub_value}'", idx)

                    elif sub_action == "select":
                        sel_el = target_cell.locator("select").first
                        sel_el.select_option(sub_value, timeout=timeout)
                        log(run_id, "info", f"[table_multi_action] row {row_idx+1} col {col} selected '{sub_value}'", idx)
                        ...
                        page.wait_for_timeout(1500)
                        opt = page.locator(
                            f"[role='option']:has-text('{sub_value}'), .ng-option:has-text('{sub_value}')"
                        ).first
                        opt.click(timeout=timeout)
                        log(run_id, "info", f"[table_multi_action] row {row_idx+1} col {col} search_selected '{sub_value}'", idx)

                    elif sub_action == "get_text":
                        txt = (target_cell.inner_text(timeout=timeout) or "").strip()
                        if store_as and resolved_vars is not None:
                            resolved_vars[store_as] = txt
                        log(run_id, "info", f"[table_multi_action] row {row_idx+1} col {col} text='{txt}'", idx)

                    elif sub_action == "assert_text":
                        txt = (target_cell.inner_text(timeout=timeout) or "").strip()
                        assert sub_value.lower() in txt.lower(), \
                            f"table_multi_action assert_text: expected '{sub_value}' in '{txt}'"
                        log(run_id, "info", f"[table_multi_action] row {row_idx+1} col {col} assert_text passed", idx)

                    elif sub_action == "assert_visible":
                        assert target_cell.is_visible(timeout=timeout), \
                            f"table_multi_action assert_visible: col {col} not visible at row {row_idx+1}"
       ...
       #     assert value == actual, f"Expected value '{value}', got '{actual}'"
            ...
            # ── AI VISUAL ASSERTION — screenshot current page, ask Claude if it looks correct
            question = apply_variables(step.get("value", ""), resolved_vars)
            if not question.strip():
                raise Exception("assert_ai: question is empty")

            # Take screenshot of current active page
            import base64 as _b64, tempfile as _tmp, os as _os
            tmp_path = _os.path.join(SCREENSHOTS_DIR, f"{run_id}_assert_ai_{idx+1}.png")
            page.screenshot(path=tmp_path, full_page=True)
            with open(tmp_path, "rb") as f:
                img_b64 = _b64.b64encode(f.read()).decode()

            # Ask Claude with the screenshot
            import requests as _req, json as _json
            api_key = os.environ.get("ANTHROPIC_API_KEY", "")
            prompt = f"""You are a QA automation assistant verifying a healthcare application.
Look at this screenshot carefully and answer the following question with YES or NO, then explain briefly.
...
REASON: <what is wrong or missing>"""
            ...
            if claude_resp.status_code != 200:
                raise Exception(f"assert_ai: Claude API error {claude_resp.status_code}: {claude_resp.text[:200]}")

            resp_text = claude_resp.json()["content"][0]["text"].strip()
            log(run_id, "info", f"  [assert_ai] Claude response: {resp_text[:300]}", idx)

            # Parse RESULT and REASON
            ...
            passed = "YES" in result_line.upper()
            reason = reason_line.replace("REASON:", "").strip() if reason_line else resp_text[:200]
            ...
            # ── AI VISUAL EXTRACT — screenshot current page, extract a value into a variable
            import base64 as _b64, os as _os, requests as _req

            # Safely get question — handle if value is a dict
            raw_question = step.get("value", "") or ""
            if isinstance(raw_question, dict):
                raw_question = raw_question.get("value", "") or ""
            question = apply_variables(str(raw_question), resolved_vars)

            # Safely get store_as — handle if store_as is a dict
            raw_store_as = step.get("store_as", "") or ""
            if isinstance(raw_store_as, dict):
                raw_store_as = raw_store_as.get("value", "") or ""
            store_as = str(raw_store_as).strip()
            if store_as.startswith("{{") and store_as.endswith("}}"):
                store_as = store_as[2:-2].strip()

            if not question.strip():
                raise Exception("ai_extract: question/prompt is empty")
            if not store_as:
                raise Exception("ai_extract: store_as variable name is empty")

            tmp_path = _os.path.join(SCREENSHOTS_DIR, f"{run_id}_ai_extract_{idx+1}.png")
            page.screenshot(path=tmp_path, full_page=False)
            with open(tmp_path, "rb") as f:
                img_b64 = _b64.b64encode(f.read()).decode()

            api_key = os.environ.get("ANTHROPIC_API_KEY", "")
            prompt = ("You are a QA automation assistant extracting data from a healthcare application screenshot.\n\n"
                      f"Look at this screenshot carefully and extract the following:\n{question}\n\n"
                      "Respond with ONLY the extracted value — no explanation, no label, no punctuation around it.\n"
                      "If the value is not found on the screen, respond with: NOT_FOUND")
            ...
            if claude_resp.status_code != 200:
                raise Exception(f"ai_extract: Claude API error {claude_resp.status_code}: {claude_resp.text[:200]}")

            extracted = claude_resp.json()["content"][0]["text"].strip()
            log(run_id, "info", f"  [ai_extract] Claude extracted: '{extracted[:100]}'", idx)

            if extracted == "NOT_FOUND":
                raise AssertionError(f"ai_extract: value not found on screen for: '{question[:80]}'")

            if resolved_vars is not None:
                resolved_vars[store_as] = extracted
            log(run_id, "pass", f"[OK] ai_extract: '{question[:60]}' -> '{extracted[:80]}' stored in {{{{{store_as}}}}}", idx)

        elif action == "screenshot":
            take_screenshot(page, run_id, value or f"step_{idx+1}")

        elif action == "scroll":
            scroll_x = int(step.get("value2", 0) or 0)
            scroll_y = int(value or 0)
            scroll_sel = selector.strip() if selector else ""
            log(run_id, "info", f"  [scroll] scrolling to X={scroll_x} Y={scroll_y} selector={scroll_sel or 'auto'}", idx)
            if scroll_sel:
                # Scroll a specific element by selector using JS
                page.evaluate(f"""
                    (function() {{
                        var el = document.querySelector({repr(scroll_sel)});
                        if (!el) el = document.documentElement;
                        if (el.scrollTo) el.scrollTo({scroll_x}, {scroll_y});
                        else {{ el.scrollLeft = {scroll_x}; el.scrollTop = {scroll_y}; }}
                    }})();
                """)
            else:
                # Auto-detect scrollable container and set scrollLeft/scrollTop directly
                # .scroll-table-container is the known scrollable div in Narayana app
                page.evaluate(f"""
                    (function() {{
                        var scrolled = false;
                        // Priority 1: known app-specific containers
                        var known = [
                        ...
                        for (var k = 0; k < known.length; k++) {{
                            var el = document.querySelector(known[k]);
                            if (el) {{
                                el.scrollLeft = {scroll_x};
                                el.scrollTop  = {scroll_y};
                                scrolled = true;
                                break;
                            }}
                        }}
                        // Priority 2: any element with actual horizontal overflow
                        if (!scrolled) {{
                            ...
                            for (var i = 0; i < all.length; i++) {{
                                ...
                                try {{
                                    ...
                                    if ((s.overflowX === 'auto' || s.overflowX === 'scroll') &&
                                        ...
                                        el.scrollLeft = {scroll_x};
                                        if ({scroll_y} !== 0) el.scrollTop = {scroll_y};
                                        scrolled = true;
                                        break;
                                    }}
                                }} catch(e) {{}}
                        ...
                        document.documentElement.scrollLeft = {scroll_x};
                    }})();
                """)
            ...
            # Run query, map each column of first row to a variable
            db_result = run_db_extract_multi(step, run_id, idx, resolved_vars)
            if db_result["status"] == "failed":
                return db_result
            if resolved_vars is not None and "stored_vars" in db_result:
                resolved_vars.update(db_result["stored_vars"])

        elif action == "compare_pdf_page":
            # Compare PDF content with web page content line by line
            # value    = full path to PDF file (e.g. C:\Users\...\terms.pdf)
            # selector = CSS selector of element (blank = full body)
            import re as _re_pdf, unicodedata as _ud

            pdf_path = value.strip()
            log(run_id, "info", f"[compare_pdf_page] Reading PDF: {pdf_path}", idx)

            # ── Wait for page to fully load (critical after switch_window) ──
            try:
            ...
            try:
                ...
                with pdfplumber.open(pdf_path) as _pdf:
                    _pdf_page_count = len(_pdf.pages)
                    for _page_obj in _pdf.pages:
                        _t = _page_obj.extract_text(x_tolerance=2, y_tolerance=2)
                        if _t:
                            pdf_text += _t + "\n"
                log(run_id, "info", f"[compare_pdf_page] PDF: {_pdf_page_count} pages, {len(pdf_text)} chars extracted", idx)
            except ImportError:
                raise Exception("pdfplumber not installed. Run: pip install pdfplumber --break-system-packages")
            except Exception as _e:
                raise Exception(f"Could not read PDF '{pdf_path}': {_e}")
            ...
            if selector and selector.strip():
                page_text = get_locator(page, selector).inner_text(timeout=timeout)
            else:
                page_text = page.inner_text("body")
            log(run_id, "info", f"[compare_pdf_page] Page: {len(page_text)} chars extracted", idx)
            ...
            def _norm_text(t):
                ...
                lines = [_re_pdf.sub(r"[ \t]+", " ", l).strip() for l in t.splitlines()]
                # Remove empty lines and very short lines (page numbers, headers)
                lines = [l for l in lines if len(l) >= 15]
                return lines

            pdf_lines  = _norm_text(pdf_text)
            page_lines = _norm_text(page_text)
            page_full  = " ".join(page_lines).lower()  # full page as single string for search

            log(run_id, "info", f"[compare_pdf_page] PDF lines: {len(pdf_lines)} | Page lines: {len(page_lines)}", idx)

            # ── Compare line by line ──────────────────────────────────────
            # For each PDF line: check if it exists anywhere on the page (exact, case-insensitive)
            matched   = []
            missing   = []
            for _line in pdf_lines:
                if _line.lower() in page_full:
                    matched.append(_line)
                else:
                    missing.append(_line)

            # Lines on page not found in PDF
            pdf_full = " ".join(pdf_lines).lower()
            extra = [l for l in page_lines if l.lower() not in pdf_full]

            # ── Report ────────────────────────────────────────────────────
            _pct = int(len(matched) / len(pdf_lines) * 100) if pdf_lines else 0
            log(run_id, "pass" if not missing else "warn",
                f"[compare_pdf_page] Match: {len(matched)}/{len(pdf_lines)} lines ({_pct}%)", idx)

            if not missing:
                log(run_id, "pass", "[compare_pdf_page] ✅ PERFECT MATCH -- all PDF content found on page", idx)
            else:
                log(run_id, "warn", f"[compare_pdf_page] ⚠️  {len(missing)} line(s) from PDF NOT found on page:", idx)
                for _i, _line in enumerate(missing, 1):
                    _preview = _line[:150] + ("..." if len(_line) > 150 else "")
                    log(run_id, "warn", f"[compare_pdf_page]   [{_i}] ❌ {_preview}", idx)

            if extra:
                log(run_id, "info", f"[compare_pdf_page] ℹ️  {len(extra)} line(s) on page NOT in PDF:", idx)
                for _i, _line in enumerate(extra[:10], 1):
                    _preview = _line[:150] + ("..." if len(_line) > 150 else "")
                    log(run_id, "info", f"[compare_pdf_page]   [{_i}] ➕ {_preview}", idx)

            log(run_id, "info",
                f"[compare_pdf_page] Summary -- PDF: {len(pdf_lines)} lines | "
                f"Matched: {len(matched)} | Missing: {len(missing)} | Extra on page: {len(extra)}", idx)
            # Always pass -- tester reviews the diff report and decides

        # ─────────────────────────────────────────────────────────────────────────
        # VISUAL TESTING STEPS — NEW KEYWORDS (do not touch existing keywords)
        # visual_checkpoint  — compare current screen vs stored baseline
        # visual_figma_check — compare current screen vs Figma design frame
        # ─────────────────────────────────────────────────────────────────────────
            ...
            match_level  = step.get("match_level", "ai").lower()  # ai | layout | content | strict
            ignore_text  = step.get("ignore_text", "")            # comma-separated patterns to ignore
            api_key      = os.environ.get("ANTHROPIC_API_KEY", "")

            # Folders — completely separate from runner/screenshots
            ...
            ts          = int(time.time())
            actual_path = os.path.join(compare_dir, f"actual_{safe_name}_{run_id}_{ts}.png")
            diff_path   = os.path.join(compare_dir, f"diff_{safe_name}_{run_id}_{ts}.png")

            # Helper: send image to run report (only called on failure)
            def _send_visual_image(filepath, label):
                try:
                    with open(filepath, "rb") as _f:
                        _b64data = _b64.standard_b64encode(_f.read()).decode()
                    requests.post(f"{API_BASE}/api/runs/{run_id}/screenshot", json={
                        "label":     label,
                    ...
                    log(run_id, "info", f"[visual] Could not send image to report: {_e}", idx)

            # ── Step 1: Screenshot current page ────────────────────────────────
            ...
            if action == "visual_figma_check" and figma_url and figma_token:
                ...
                node_id  = _ulp.unquote(_fmatch.group(2)).replace("-", ":")
                figma_api_url = f"https://api.figma.com/v1/images/{file_key}?ids={_ulp.quote(node_id)}&format=png&scale=2"
                figma_resp = requests.get(figma_api_url, headers={"X-Figma-Token": figma_token}, timeout=30)
                figma_data = figma_resp.json()
                img_url = figma_data.get("images", {}).get(node_id) or \
                          figma_data.get("images", {}).get(node_id.replace(":", "-"))
                if not img_url:
                    raise Exception("visual_figma_check: Figma returned no image URL")
                figma_png_path = os.path.join(compare_dir, f"figma_{safe_name}_{ts}.png")
                img_data = requests.get(img_url, timeout=30)
                with open(figma_png_path, "wb") as f:
                    f.write(img_data.content)
                expected_path = figma_png_path
                log(run_id, "info", "[visual] Figma frame downloaded", idx)

            else:
                baseline_path = os.path.join(baseline_dir, f"baseline_{safe_name}.png")
                if not os.path.exists(baseline_path):
                    # First run — save as baseline, pass without comparison
                    _shutil.copy(actual_path, baseline_path)
                    log(run_id, "pass",
                        f"[visual] FIRST RUN — baseline saved as '{safe_name}'. "
                        f"Future runs will compare against this baseline.", idx)
                    expected_path = None
                else:
                    expected_path = baseline_path
                    log(run_id, "info", f"[visual] Using baseline: {os.path.basename(baseline_path)}", idx)

            # ── Step 3: Claude Vision comparison (skip if first run) ────────────
            if expected_path is not None:
                ...
                def _img_b64(path):
                    with open(path, "rb") as f:
                        return _b64.standard_b64encode(f.read()).decode()
                ...
                if match_level == "layout":
                    prompt = (
                        "You are a senior QA engineer reviewing two UI screenshots.\n"
                        "Image 1 = EXPECTED | Image 2 = ACTUAL\n"
                        "MATCH LEVEL: LAYOUT ONLY — COMPLETELY IGNORE all text, numbers, names, dates, IDs.\n"
                        "Only flag differences in element positions, sizes, visibility, layout shifts, missing/extra UI elements."
                        + ignore_extra +
                        "\nRespond ONLY in valid JSON (no markdown):\n"
                        '{"differences":[{"severity":"CRITICAL","element":"name","description":"issue"}],' +
                        '"summary":"one sentence","critical_count":0,"minor_count":0,"cosmetic_count":0}'
                    )
                elif match_level == "content":
                    prompt = (
                        "You are a senior QA engineer reviewing two UI screenshots.\n"
                        "Image 1 = EXPECTED | Image 2 = ACTUAL\n"
                        "MATCH LEVEL: CONTENT ONLY — check static labels, headings, button text only.\n"
                        "Ignore colours, fonts, spacing, dynamic data (patient IDs, dates, names, amounts)."
                        + ignore_extra +
                        "\nRespond ONLY in valid JSON (no markdown):\n"
                        '{"differences":[{"severity":"CRITICAL","element":"name","description":"issue"}],' +
                        '"summary":"one sentence","critical_count":0,"minor_count":0,"cosmetic_count":0}'
                    )
                elif match_level == "strict":
                    prompt = (
                        "You are a senior QA engineer reviewing two UI screenshots.\n"
                        "Image 1 = EXPECTED | Image 2 = ACTUAL\n"
                        "MATCH LEVEL: STRICT — check everything including all text, colours, spacing."
                        + ignore_extra +
                        "\nRespond ONLY in valid JSON (no markdown):\n"
                        '{"differences":[{"severity":"CRITICAL","element":"name","description":"issue"}],' +
                        '"summary":"one sentence","critical_count":0,"minor_count":0,"cosmetic_count":0}'
                    ...
                    prompt = (
                        "You are a senior QA engineer reviewing two UI screenshots.\n"
                        "Image 1 = EXPECTED | Image 2 = ACTUAL\n"
                        "MATCH LEVEL: AI (smart) — compare like a human QA engineer.\n"
                        "Automatically IGNORE: patient names, patient IDs, dates, times, "
                        "transaction IDs, amounts, usernames, appointment numbers, bed numbers.\n"
                        "Focus on: layout, structure, colours, fonts, button labels, menu items, form fields, missing elements."
                        + ignore_extra +
                        "\nClassify each real difference as CRITICAL / MINOR / COSMETIC.\n"
                        "Respond ONLY in valid JSON (no markdown):\n"
                        '{"differences":[{"severity":"CRITICAL","element":"name","description":"what differs"}],' +
                        '"summary":"one sentence","critical_count":0,"minor_count":0,"cosmetic_count":0}'
                    )
                ...
                if claude_resp.status_code != 200:
                    raise Exception(f"visual_checkpoint: Claude API error {claude_resp.status_code}: {claude_resp.text[:300]}")

                claude_text = claude_resp.json()["content"][0]["text"].strip()
                log(run_id, "info", f"[visual] Claude response: {claude_text[:200]}", idx)
                ...
                claude_result = _json.loads(claude_text)

                # ── Step 4: Pixel diff — calculate mismatch % ───────────────────
                diff_pct     = 0
                diff_created = False
                try:
                    ...
                    exp_img = _PILImage.open(expected_path).convert("RGB")
                    act_img = _PILImage.open(actual_path).convert("RGB")
                    if exp_img.size != act_img.size:
                        act_img = act_img.resize(exp_img.size, _PILImage.LANCZOS)

                    exp_arr   = _np.array(exp_img, dtype=_np.float32)
                    act_arr   = _np.array(act_img, dtype=_np.float32)
                    diff_arr  = _np.abs(exp_arr - act_arr)
                    diff_mask = _np.mean(diff_arr, axis=2) > 15
                    diff_pct  = round(float(diff_mask.sum()) / diff_mask.size * 100, 2)

                except ImportError:
                    log(run_id, "info", "[visual] Pillow not installed — install: pip install Pillow numpy", idx)
                ...
                if visual_failed:
                    # ── ONLY ON FAILURE: create highlighted diff image and send to report ──
                    try:
                        from PIL import Image as _PILImage, ImageDraw as _PILDraw
                        import numpy as _np
                        ...
                        # Create highlighted diff — red overlay on changed pixels in actual
                        act_highlighted = act_img.copy()
                        act_arr_u8      = _np.array(act_highlighted)
                        # Paint changed pixels red
                        act_arr_u8[diff_mask] = [255, 60, 60]
                        act_highlighted = _PILImage.fromarray(act_arr_u8.astype(_np.uint8))

                        # Scale down if too wide
                        w, h   = exp_img.size
                        scale  = min(1.0, 580 / w)
                        pw, ph = int(w * scale), int(h * scale)
                        exp_r  = exp_img.resize((pw, ph), _PILImage.LANCZOS)
                        act_r  = act_highlighted.resize((pw, ph), _PILImage.LANCZOS)

                        # Side-by-side: Expected | Actual (with red highlights on changes)
                        header_h = 52
                        canvas   = _PILImage.new("RGB", (pw * 2 + 6, ph + header_h), (30, 30, 30))
                        draw     = _PILDraw.Draw(canvas)

                        # Header — left: expected, right: actual with changes highlighted
                        draw.rectangle([0,    0, pw,      header_h - 1], fill=(26, 111, 196))
                        draw.rectangle([pw+6, 0, pw*2+6,  header_h - 1], fill=(180, 30, 30))
                        draw.text((8,    8),  "EXPECTED",                              fill="white")
                        draw.text((8,    26), os.path.basename(expected_path)[:38],    fill=(180, 215, 255))
                        draw.text((pw+14, 8),  f"ACTUAL  — changes highlighted in red", fill="white")
                        draw.text((pw+14, 26), f"Pixel diff: {diff_pct}%  |  Threshold: {threshold}%", fill=(255, 180, 180))

                        canvas.paste(exp_r, (0,    header_h))
                        canvas.paste(act_r, (pw+6, header_h))
                        canvas.save(diff_path)
                        diff_created = True

                        # Send only the diff image — tester sees changes highlighted in red
                        _send_visual_image(diff_path,
                            f"[visual FAIL] {visual_name} — {diff_pct}% diff — red = changed pixels")

                    except ImportError:
                        # Pillow not available — send actual + expected separately
                        log(run_id, "info", "[visual] Pillow not installed — sending raw screenshots instead", idx)
                        _send_visual_image(expected_path, f"[visual FAIL] Expected — {visual_name}")
                        _send_visual_image(actual_path,   f"[visual FAIL] Actual — {visual_name}")
                    except Exception as _pe:
                        log(run_id, "info", f"[visual] Diff image error: {_pe}", idx)
                        _send_visual_image(expected_path, f"[visual FAIL] Expected — {visual_name}")
                        _send_visual_image(actual_path,   f"[visual FAIL] Actual — {visual_name}")

                    # Build failure message with Claude's diff list
                    diff_lines = "\n".join(
                        [f"  [{d['severity']}] {d['element']}: {d['description']}" for d in diffs]
                    )
                    raise AssertionError(
                        f"[visual] FAILED — {summary}\n"
                        f"Pixel mismatch: {diff_pct}% (threshold: {threshold}%)\n"
                        + (f"Claude found {len(diffs)} difference(s):\n{diff_lines}" if diffs else "")
                    )

                else:
                    # PASS — no images sent, clean log only
                    minor    = claude_result.get("minor_count", 0)
                    cosmetic = claude_result.get("cosmetic_count", 0)
                    log(run_id, "pass",
                        f"[visual] PASSED — {summary} | "
                        f"Pixel diff: {diff_pct}% | Minor: {minor} | Cosmetic: {cosmetic}", idx)
        ...
        pass_desc = {
            ...
            "press_sequentially": f"PASSED -- Typed '{value}' into debounce field: [ {selector} ]",
            ...
            "scroll":             f"PASSED -- Scrolled to X={step.get('value2', 0) or 0} Y={value or 0} px",
            "execute_script":     f"PASSED -- JavaScript executed successfully on the page",
            "db_validate":        f"PASSED -- DB validation query passed",
            "db_extract_multi":   f"PASSED -- DB extracted {len(step.get('db_config',{}).get('mappings',[]))} column(s) into variables",
        ...
        if page and action_used not in ("assert_text", "assert_visible",
            ...
            # ── HEAL CACHE: check in-memory cache first (loaded at startup, no DB call per step)
            original_selector = step.get("selector", "")
            if original_selector and original_selector in _heal_cache:
                cached_healed = _heal_cache[original_selector]
                try:
                    loc = get_locator(page, cached_healed)
                    loc.wait_for(state="attached", timeout=3000)
                    healed_selector = cached_healed
                    log(run_id, "pass", f"[⚡] Heal cache HIT — skipped AI, used cached selector", idx)
                except Exception:
                    log(run_id, "info", f"  [heal cache] cached selector stale, calling AI", idx)
            ...
            if healed_selector:
                ...
                # ── HEAL CACHE: update in-memory dict + persist to DB for future runner restarts
                original_selector = step.get("selector", "")
                if original_selector and healed_selector:
                    _heal_cache[original_selector] = healed_selector  # instant, in-memory
                    try:
                        requests.post(
                            f"{API_BASE}/api/heal-cache",
                            json={"original": original_selector, "healed": healed_selector},
                            headers={"Authorization": f"Bearer {RUNNER_TOKEN}"},
                            timeout=3
                        )
                    except Exception:
                        pass  # DB persist failure is silent — in-memory still works this session
                # Save healed selector to DB only if heal_update is enabled on this test case
                test_case_id = step.get("_test_case_id")
                if test_case_id and heal_update:
                    try:
                        resp = requests.patch(
                            f"{API_BASE}/api/tests/{test_case_id}/heal/{idx}",
                        ...
                        if resp.status_code == 200:
                            ...
                            log(run_id, "warn", f"  [AI Heal] DB save failed ({resp.status_code}): {resp.text[:100]}", idx)
...
#----DB Multi----------------------------
def run_db_extract_multi(step, run_id, idx, resolved_vars=None):
    """Run one query, store each column of the first row into a separate variable."""
    cfg = step.get("db_config", {})
    if not cfg:
        log(run_id, "fail", "[FAIL] db_extract_multi: no db_config on step", idx)
        return {"status": "failed", "step": idx, "error": "No db_config found"}
    ...
    if not query.strip():
        log(run_id, "fail", "[FAIL] db_extract_multi: no query defined", idx)
        return {"status": "failed", "step": idx, "error": "No SQL query defined"}
    if not mappings:
        log(run_id, "fail", "[FAIL] db_extract_multi: no column mappings defined", idx)
        return {"status": "failed", "step": idx, "error": "No column mappings defined"}

    log(run_id, "info", f"  [db_extract_multi] Query: {query[:120]}{'...' if len(query)>120 else ''}", idx)
    ...
    if conn_mode == "manual":
        ...
        if not host or not database or not user:
            msg = "Manual DB connection requires host, database and username"
            log(run_id, "fail", f"[FAIL] {msg}", idx)
            return {"status": "failed", "step": idx, "error": msg}
        conn_cfg = {"host": host, "port": port, "database": database, "user": user, "password": password}
        log(run_id, "info", f"  [db_extract_multi] Using manual: {user}@{host}:{port}/{database}", idx)
    else:
        conn_name = cfg.get("conn_name", "").strip()
        if not conn_name:
            msg = "No saved connection name provided"
            log(run_id, "fail", f"[FAIL] {msg}", idx)
            return {"status": "failed", "step": idx, "error": msg}
        log(run_id, "info", f"  [db_extract_multi] Using saved connection: '{conn_name}'", idx)
        try:
    ...
    try:
        rows, col_names = execute_db_query("postgresql", conn_cfg, query)
    except Exception as e:
        log(run_id, "fail", f"[FAIL] db_extract_multi query error: {e}", idx)
        return {"status": "failed", "step": idx, "error": f"DB query error: {e}"}

    if not rows:
        log(run_id, "fail", "[FAIL] db_extract_multi: query returned no rows", idx)
        return {"status": "failed", "step": idx, "error": "Query returned no rows"}

    # rows are dicts keyed by column alias
    row = rows[0]
    log(run_id, "info", f"  [db_extract_multi] Row columns: {list(row.keys())}", idx)

    stored = {}
    for map_idx, m in enumerate(mappings):
        ...
        try:
            if col_name and col_name in row:
                ...
                keys = list(row.keys())
                val = row[keys[map_idx]] if map_idx < len(keys) else ""
            else:
                log(run_id, "warn", f"  [db_extract_multi] ⚠️ Column '{col_name}' not found. Available: {list(row.keys())}", idx)
                val = ""
        except Exception as e:
            log(run_id, "warn", f"  [db_extract_multi] ⚠️ Error on '{col_name}': {e}", idx)
            val = ""
        stored[var_name] = str(val) if val is not None else ""
        log(run_id, "info", f"  [db_extract_multi] {{{var_name}}} = '{stored[var_name]}'", idx)

    log(run_id, "pass", f"[OK] db_extract_multi: stored {len(stored)} variable(s)", idx)
    return {"status": "passed", "step": idx, "stored_vars": stored}
def run_db_validate(step, run_id, idx, resolved_vars=None):
    ...
    try:
        if assert_type == "is_empty":
            ...
            # If cell is a dict/list (e.g. PostgreSQL JSONB column), serialize to proper JSON string
            if isinstance(cell, (dict, list)):
                stored[var_name] = _json_store.dumps(cell)
            else:
                stored[var_name] = str(cell)
            log(run_id, "pass", f"[OK] PASSED -- Stored '{str(stored[var_name])[:60]}' into variable {{{{ {var_name} }}}}", idx)
...
def run_api_test(config, run_id):
    ...
    variables = {**project_vars, **test_vars}  # test vars override project vars
    ...
    if auth_type == "bearer":
        ...
        if token:
            ...
            log(run_id, "warn", "[AUTH] Bearer auth selected but token is empty -- check {{Token}} variable")
        ...
        if key_name and key_value:
            ...
            log(run_id, "warn", "[AUTH] API key header auth: key_name or key_value missing")
        ...
        if param_name and key_value:
            ...
            log(run_id, "warn", "[AUTH] API key query auth: param_name or key_value missing")
    ...
    try:
        ...
        try:
            ...
            # Log full body so all fields are visible in run logs
            log(run_id, "info", f"Response Body: {response_body}")
        except:
            pass
        # Run assertions
            ...
            try:
                if atype == "status_code":
                    ...
                    if var_name:
                        ...
                        # Also persist into project_vars so it gets written back to DB
                        project_vars[var_name] = variables[var_name]
                    ...
                    if var_name and key:
                        ...
                        project_vars[var_name] = val
                        log(run_id, "info", f"📝 [EXTRACT HEADER] {var_name} = {val}")

                elif atype == "extract_cookie":
                    var_name = assertion.get('variable', '')
                    key      = assertion.get('key', '')
                    if var_name and key:
                        ...
                        variables[var_name] = val
                        project_vars[var_name] = val
                        log(run_id, "info", f"📝 [EXTRACT COOKIE] {var_name} = {val}")
    ...
    if project_id and project_vars:
        try:
            ...
            if runtime_updates:
                ...
                base = _os.environ.get("API_BASE_URL", "http://localhost:6001").split('#')[0].strip()
                _req.patch(
...
def evaluate_condition(page, step, resolved_vars):
    ...
    try:
        if cond == "element_visible":
            ...
            #print(f"[IF DEBUG] var={var_name} actual='{actual}' expected='{expected}' result={result}")
            #return result
...
def run_steps_with_flow(page, steps, run_id, resolved_vars, config, continue_on_fail=False, variables=None):
    ...
    while i < n:
        ...
        if step.get("disabled"):
            log(run_id, "info", f"[SKIP] Step {i+1} ({action}) is disabled -- skipping")
            i += 1
            continue
            ...
            try:
                sel = apply_variables(step.get("selector",""), resolved_vars)
                    ...
                    #stored = str(get_locator(page, sel).get_attribute(attr) or "") 
                      ...
                      # stored = str(get_locator(page, sel).evaluate("el => el.className") or "")
                    #else:
                       #stored = str(get_locator(page, sel).get_attribute(attr) or "")
                if action == "store_attr":
                    #attr = apply_variables(step.get("value2","") or step.get("value",""), resolved_vars)
        ...
        if action == "get_table_value":
            ...
            try:
                ...
                # Comparison mismatch -- log as SOFT FAIL and continue by default
                # Only hard-stop if continue_on_fail is explicitly set to False
                log(run_id, "warn", f"[SOFT FAIL] {e}")
                results.append({"status":"failed", "step":i, "error":str(e), "soft":True})
                if step.get("continue_on_fail") is False:
                    return results
        ...
        if action in ("str_upper","str_lower","str_trim","str_replace","str_substring",
            ...
            # store_as: strip {{ }} braces if user typed them, then use as var name
            raw_store_as = step.get("store_as", "").strip()
            if raw_store_as.startswith("{{") and raw_store_as.endswith("}}"):
                raw_store_as = raw_store_as[2:-2].strip()
            var_name = raw_store_as or src_var
            try:
        ...
        if action == "call_test":
            ...
            if not called_id:
                log(run_id, "warn", "[call_test] No test case ID specified -- skipping")
                i += 1; continue

            # ── Depth limit -- prevent infinite call chains ────────────────────
            MAX_CALL_DEPTH = 5
            call_stack = config.get("__call_stack__", [])
            current_test_id = str(config.get("test_case_id",""))

            if len(call_stack) >= MAX_CALL_DEPTH:
                msg = f"call_test exceeded max depth ({MAX_CALL_DEPTH}): {' → '.join(call_stack)} → {called_id}"
                log(run_id, "fail", f"[FAIL] {msg}")
                results.append({"status":"failed","step":i,"error":msg})
                if not continue_on_fail: return results
                i += 1; continue
            ...
            if called_id in call_stack or called_id == current_test_id:
                msg = f"call_test circular reference detected: {' → '.join(call_stack + [current_test_id])} → {called_id}"
                log(run_id, "fail", f"[FAIL] {msg}")
                results.append({"status":"failed","step":i,"error":msg})
                if not continue_on_fail: return results
                i += 1; continue

            log(run_id, "info", f">> Calling test case ID: {called_id} (depth {len(call_stack)+1}/{MAX_CALL_DEPTH})")
            try:
                # Fetch the called test case from server
                resp = requests.get(
                    f"{API_BASE}/api/tests/{called_id}",
                    headers={"Authorization": f"Bearer {RUNNER_TOKEN}"},
                    timeout=15
                )
                ...
                # ── Cross-project check -- warn but allow (backward compatible) ──
                caller_project = config.get("project_id")
                called_project = called_test.get("project_id")
                if caller_project and called_project and str(caller_project) != str(called_project):
                    log(run_id, "warn", f"[call_test] Warning: cross-project call -- caller project={caller_project}, called project={called_project}")

                # ── Callable flag check -- warn but allow (backward compatible) ──
                if not called_test.get("is_callable"):
                    log(run_id, "warn", f"[call_test] Warning: test \'{called_test.get('name','?')}\' is not marked as callable -- consider enabling it")

                called_steps = called_test.get("steps") or []
                called_vars  = called_test.get("variables") or []
                called_name  = called_test.get("name","Unknown")

                if not called_steps:
                    log(run_id, "warn", f"[call_test] '{called_name}' has no steps -- skipping")
                    i += 1; continue

                log(run_id, "info", f"[call_test] Running '{called_name}' ({len(called_steps)} steps) inline")

                # Deep-copy called steps and inject _test_case_id
                # so AI Heal saves back to the CHILD test, not the parent
                import copy as _copy
                called_steps = _copy.deepcopy(called_steps)
                for cs in called_steps:
                    cs["_test_case_id"] = called_id

                # Deep-copy steps + inject _test_case_id so AI Heal saves to CHILD test
                import copy as _copy
                called_steps = _copy.deepcopy(called_steps)
                for cs in called_steps:
                    cs["_test_case_id"] = called_id

                # Build child config with updated call stack
                child_config = dict(config)
                child_config["__call_stack__"] = call_stack + [current_test_id]
                child_config["test_case_id"] = called_id

                # Build child vars -- parent vars override child defaults
                child_vars = dict(resolved_vars)
                # Resolve the called test's variables properly (handles data_table, dynamic, etc)
                called_resolved = resolve_variables(called_vars)
                for vname, vval in called_resolved.items():
                    if vname and vname not in child_vars:
                        child_vars[vname] = vval

                # Run called test's steps inline
                sub_results = run_steps_with_flow(
                    page, called_steps, run_id, child_vars, child_config,
                    continue_on_fail, called_vars
                )

                # Propagate variables back to parent
                for k, v2 in child_vars.items():
                    resolved_vars[k] = v2

                results.extend([r for r in sub_results if not r.get("__break__") and not r.get("__continue__")])

                failed = [r for r in sub_results if r.get("status") == "failed"]
                if failed:
                    log(run_id, "fail", f"[call_test] '{called_name}' had {len(failed)} failure(s)")
                    if not continue_on_fail:
                        return results
                else:
                    log(run_id, "pass", f"[OK] call_test: '{called_name}' completed ({len(called_steps)} steps)")

            except Exception as e:
                msg = f"call_test failed for ID '{called_id}': {e}"
                log(run_id, "fail", f"[FAIL] {msg}")
                results.append({"status":"failed","step":i,"error":msg})
                if not continue_on_fail: return results
            i += 1; continue
        ...
        if action in ("encode_base64","decode_base64","url_encode","json_extract","json_parse"):
            # For json_extract: store_as comes from 'value' field wrapped in {{}}, strip the braces
            raw_store = step.get("store_as","") or step.get("value","")
            # Strip {{ }} to get plain variable name
            ...
            try:
                ...
                if   action == "encode_base64": result_val = _b64.b64encode(src.encode()).decode()
                    ...
                    # Support both old UI (selector field) and new UI (value field) for source
                    raw_src = step.get("selector", "") or src_var
                    # Strip {{ }} if present
                    raw_src = raw_src.strip().strip("{}").strip()
                    raw_src = apply_variables(raw_src, resolved_vars)
                    # Look up the variable value
                    json_src = resolved_vars.get(raw_src, raw_src)
                    import json as _json2
                    if isinstance(json_src, (dict, list)):
                        obj = json_src
                    else:
                        try:
                            obj = _json2.loads(str(json_src))
                        except Exception:
                            import ast
                            obj = ast.literal_eval(str(json_src))
                    path = v2.split(".")
                    for key in path:
        ...
        _step_action = step.get("action","step")
        result = run_step(page, step, run_id, i, resolved_vars)
        take_live_screenshot(page, run_id, _step_action)
...
def main():
    ...
    # Load heal cache in background thread -- don't block test startup
    import threading as _threading
    _threading.Thread(target=_load_heal_cache, daemon=True).start()
    ...
    heal_update  = config.get("heal_update", False)  # if True, save healed selector to DB
    project_id   = config.get("project_id")
    ...
    try:
        if test_type == "api":
            ...
            with sync_playwright() as p:
                ...
                # Reset live screenshot dedup state for this test
                global _last_frame_hash, _last_live_ts
    ...
    final_status= "passed" if failed == 0 and len(step_results) > 0 else "failed"
